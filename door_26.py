"""
西州将军铜门 - 生产下单系统 (Pro Dashboard 高级版)
...
def main():
    ...
    history_mgr, options_mgr = HistoryManager(HISTORY_FILE), CustomOptionsManager(CUSTOM_OPTIONS_FILE)
    ...
    with col_right:
        ...
        with st.container(border=True):
            ...
            # --- 核心出图逻辑挂载 ---
            if st.button("🚀 生成生产下料CAD与BOM排版", type="primary", use_container_width=True):
                # 保存历史
                ...
                dxk_val, gxk_val, pdk_val = ("", "", st.session_state.get("pdk", "")) if ttype == "平底槛" else (st.session_state.get("th_str", "55/70").split("/")[0], st.session_state.get("th_str", "55/70").split("/")[-1], "")

                dt_cn = {"两定两开": "两定两开门"}.get(st.session_state["door_type"], st.session_state["door_type"])

...
class EzdxfDrawer:
    ...
    def draw_dim(self, p1, p2, text_pos, rotation, layer, text_override=""):
        ...
        dim = self.ms.add_linear_dim(base=text_pos, p1=p1, p2=p2, angle=math.degrees(rotation), text=text_override if text_override else "<>", dimstyle=dimstyle, dxfattribs={'layer': layer})
        dim.render()
...
def parse_batch_text(text: str) -> Dict[str, Any]:
    ...
    if "单门" in text: result["door_type"] = "单门"
        ...
        m = re.search(r"中门宽度[:=](\d+)mm?", text); if m: result["mid_door_width"] = int(m.group(1))
...
def set_custom_style():
    ...
    .sidebar-title { font-size: 18px; font-weight: 700; color: #1D1D1F; padding: 20px 0; border-bottom: 1px solid #E5E5EA; margin-bottom: 20px;}
    
    /* 输入框微缩 */
    .stTextInput input, .stNumberInput input, .stSelectbox div[data-baseweb="select"] > div {
        font-size: 13px !important; border-radius: 8px !important; min-height: 34px !important;
    ...
    /* 按钮样式 */
    .stButton > button { border-radius: 10px !important; font-weight: 600 !important; letter-spacing: 0.5px; min-height: 44px; }
    ...
    .stButton > button[kind="secondary"]:hover { background-color: #E5E5EA !important; }
    ...
    /* 列表卡片 */
    .task-card { background: #FFF; border: 1px solid #E5E5EA; border-radius: 12px; padding: 16px; margin-bottom: 12px; box-shadow: 0 1px 4px rgba(0,0,0,0.02); }
...
def init_session_state():
    if "db_tasks" not in st.session_state:
        st.session_state["db_tasks"] = [] # 模拟数据库：存储所有任务字典
    
    if "current_module" not in st.session_state:
        st.session_state["current_module"] = "信息录入"
...
def load_task_to_session(task):
    """将任务数据加载到当前表单 Session 中"""
    for k, v in task["params"].items():
        st.session_state[k] = v

def get_current_form_data():
    """从 Session 中提取当前表单的所有数据"""
...
def generate_cad_trigger(history_mgr):
    ...
    info_map = {
        ...
        "ST": st.session_state["st_val"], "HYSL": st.session_state["hysl"], "QH": f"{st.session_state['qh']} mm" if st.session_state['qh'] else "",
        ...
        "HAS_PILLAR": st.session_state["has_pillar"], "HYYS": st.session_state["sel_hys"], "DXK": dxk_val, "GXK": gxk_val, "PXK": pdk_val, 
...
# ===================== 主页面与系统流转 =====================
def main():
    st.set_page_config(page_title="西州将军 | 图纸协同系统", layout="wide")
    init_session_state()
    set_custom_style()
    ...
    # --- 左侧导航栏 ---
    with st.sidebar:
        st.markdown("<div class='sidebar-title'>⚙️ PLM 协同系统</div>", unsafe_allow_html=True)
        nav_options = ["图纸信息录入", "图纸绘制", "图纸审核"]
        selected_nav = st.radio("导航菜单", nav_options, index=nav_options.index(st.session_state["current_module"]), label_visibility="collapsed")
        
        if selected_nav != st.session_state["current_module"]:
            st.session_state["current_module"] = selected_nav
            st.session_state["active_task_id"] = None # 切换模块时清空选中的任务
            st.rerun()

    # ==================== 模块 1：图纸信息录入 ====================
    if st.session_state["current_module"] == "图纸信息录入":
        st.markdown("### 📝 第一步：创建图纸任务")
        batch_text = st.text_input("✨ 智能文本识别", placeholder="粘贴销售聊天记录，回车自动解析并填充参数...")
        if batch_text:
    ...
    # ==================== 模块 2：图纸绘制 ====================
        ...
        if st.session_state["active_task_id"]:
            ...
            if active_task:
                ...
                with c_upload:
                    uploaded_file = st.file_uploader("📥 上传深化完成的图纸 (支持 JPG/PNG/PDF 等用于审核)", label_visibility="collapsed")
                    ...
                    with st.container():
                        ...
                        <div class="task-card">
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <div>
                                    <div class="task-title">{task['customer']} - {task['project']}</div>
                                    <div class="task-sub">门型：{task['door_type']} | 尺寸：{task['size']} | 日期：{task['date']}</div>
                                </div>
                                <div>{get_status_badge(task['status'])}</div>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                        if st.button(f"进入深化面板", key=f"btn_{task['id']}"):
                            load_task_to_session(task) # 将任务数据灌入表单
                    ...
                    with st.container():
                        ...
                        <div class="task-card">
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <div>
                                    <div class="task-title">{task['customer']} - {task['project']}</div>
                                    <div class="task-sub">门型：{task['door_type']} | 提交时间：{task['date']}</div>
                                </div>
                                <div>{get_status_badge(task['status'])}</div>
                            </div>
                        </div>
